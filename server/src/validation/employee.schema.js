...
const EmpSchema = Joi.object({
  ...
  passport_issue_date: Joi.date().iso().required().messages({
  ...
  passport_issued_by: Joi.string().trim().min(5).max(200).required().messages({
  ...
  locality: Joi.string().trim().min(2).max(100).required().messages({
  ...
  house: Joi.string().trim().max(10).required().messages({
  ...
  building: Joi.string().trim().max(10).allow('').optional().messages({
    'string.base': 'Корпус должен быть строкой',
    'string.max': 'Корпус не может превышать {#limit} символов',
  }),
  apartment: Joi.string().trim().max(10).allow('').optional().messages({
