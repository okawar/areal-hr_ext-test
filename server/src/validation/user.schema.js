...
const userSchema = Joi.object({
  ...
  login: Joi.string().trim().min(3).max(50).pattern(/^[a-zA-Z0-9_]+$/).required()
    .messages({
      ...
      'string.pattern.base': 'Логин может содержать только латинские буквы, цифры и знак подчеркивания',
      'any.required': 'Логин обязателен для заполнения'
    }),
  password: Joi.when('$isUpdate', {
    is: true,
    then: Joi.string().min(6).max(100).allow('').optional()
      .messages({
    ...
    otherwise: Joi.string().min(6).max(100).required()
      .messages({
        'string.base': 'Пароль должен быть строкой',
